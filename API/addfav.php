...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['id_product']) && isset($_POST['id_user'])) {
        ...
        $check_query = "SELECT * FROM favorite WHERE id_product = '$id_product' AND id_user = '$id_user'";
        ...
        if ($check_result->num_rows > 0) {
            ...
            $sql = "INSERT INTO favorite (id_product, id_user) VALUES ('$id_product', '$id_user')";
            if ($koneksi->query($sql) === TRUE) {
                $response['isSuccess'] = true;
                $response['message'] = "Berhasil Menambahkan ke favorite";
            } else {
                $response['isSuccess'] = false;
                $response['message'] = "Gagal Menambahkan ke favorite: " . $koneksi->error;
            }
        }
    } else {
        $response['isSuccess'] = false;
